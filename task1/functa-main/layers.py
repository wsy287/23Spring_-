...
class FiLM(nn.Module):
    ...
    def __init__(self,
        ...
        self.shift = 0.
        if modulate_scale:
            self.scale = self.create_parameter(shape=[f_in], default_initializer=nn.init.constant_(value=1.))
        if modulate_shift:
            self.shift = self.create_parameter(shape=[f_in], default_initializer=nn.init.constant_(value=0.))
...
class LatentVector(nn.Module):
    ...
    def __init__(self, latent_dim: int, batch_size: int = 1, latent_init_scale: float = 0.0):
        ...
        self.latent_vector = Parameter(torch.Tensor(batch_size, latent_dim))
...
class MetaSGDLrs(nn.Module):
    ...
    def __init__(self,
        ...
        #     default_initializer=nn.init.uniform_(*lrs_init_range))
        m = Parameter(torch.Tensor(num_lrs))
        nn.init.uniform_(m,*lrs_init_range)
        self.meta_sgd_lrs = m

    def __call__(self):
        # Clip learning rate values
        return torch.clip(self.meta_sgd_lrs, *self.lrs_clip_range)