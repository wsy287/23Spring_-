...
class LatentModulatedSiren(nn.Module):
    ...
    def __init__(self,
        ...
        self.siren_layers = nn.ModuleList(siren_layers)
    ...
    def modulate(self, x: torch.Tensor, modulations: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if 'shift' in modulations:
            ...
            # print("*************************modulations shape:", modulations['shift'].unsqueeze(1).shape)
    ...
    def __call__(self, coords, latent_vector=None):
        ...
        # Flatten coordinates
        x = torch.reshape(coords, (coords.shape[0], -1, coords.shape[-1]))
        ...
        return torch.reshape(out, list(coords.shape[:-1]) + [self.out_channels])